...
OctantR ships with an [example plugin](https://github.com/m8yng/octant/blob/main/cmd/octant-sample-plugin/main.go).
...
built-in actions which are triggered from internal Octant events, those are defined in [octant/pkg/action/actions.go](https://github.com/m8yng/octant/blob/main/pkg/action/actions.go).
...
Besides having custom actions, there are pre-existing octant actions which are registered by octant internal modules and they can be leveraged to perform an action.
For example: `action.octant.dev/apply` (https://github.com/m8yng/octant/blob/main/pkg/action/actions.go) can be used to apply a yaml configuration for a resource.
